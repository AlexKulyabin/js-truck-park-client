...
class JSTruckParkSupabaseUser extends BaseAuthUser {
  JSTruckParkSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> jSTruckParkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = JSTruckParkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
