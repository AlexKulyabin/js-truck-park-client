...
Добавить application controller для submission UI state: single-flight submit,
typed failure state и безопасное локализованное сообщение. После этого можно
решать, мигрировать ли старый `add_parking` или удалить его только после
подтверждения usage.
