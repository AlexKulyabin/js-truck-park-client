...
| `get_filtered_parkings`; `map/home_page/home_page_widget.dart`, `create_parking2/select_parking/select_parking_widget.dart` | POST: ровно 18 params: center/radius, capacity bounds, viewport bounds, six amenity flags, `zoom_level`, lower-case `search_query`, `is_filter_active` | Home проходит через typed repository/controller и только на presentation boundary адаптируется в legacy map shape; SelectParking пока остаётся `dynamic` | Home имеет generation guard и redacted typed failures; SelectParking всё ещё использует generated direct call, `succeeded ?? true` и не защищён от stale response | SECURITY DEFINER без fixed search path; wrapper всегда anonymous; approved/admin, но не active; zoom grid; no hard result limit/order | критический; characterization: `docs/map_read_contract_characterization.md`, Home integration: `docs/home_map_read_integration.md` |
