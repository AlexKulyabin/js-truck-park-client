...
| `features/profile/data/user_profile_service.dart` -> `profile/edit_profile/edit_profile_widget.dart` / initial form | `users` | select where id, limit 1 | typed `UserProfile` list | FutureBuilder spinner; empty row supported; no explicit DB catch | средне-высокий |
...
| `features/profile/data/user_profile_service.dart` (prepared, not wired) | future profile/avatar update boundary | typed command validation only; `profileUpdate` capability disabled | `PreparedUserProfileUpdate` / future `UserProfile` | no Supabase update gateway; gateway requires atomic execution or full compensation | заблокирован до backend stage |
