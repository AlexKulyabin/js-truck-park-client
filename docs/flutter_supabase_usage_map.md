...
|---|---|---|---|---|---|
| `get_parkings_by_viewport`; caller не найден, wrapper только объявлен | POST: `min_lng`, `min_lat`, `max_lng`, `max_lat`, `zoom_level` | JSON list; accessors ожидают `lat: double`, `lng: double`, `count: int`, `id: String`, `is_cluster: bool`; outer type `ApiCallResponse` | cluster `id=null`, но accessor удаляет null отдельно и ломает соответствие parallel arrays; реального caller error flow нет | invoker; approved+active; clusters до zoom 8, затем markers, limit 500 | высокий, но сейчас dormant |
| `get_filtered_parkings`; `map/home_page/home_page_widget.dart`, `create_parking2/select_parking/select_parking_widget.dart` | POST: ровно 18 params: center/radius, capacity bounds, viewport bounds, six amenity flags, `zoom_level`, lower-case `search_query`, `is_filter_active` | Home и SelectParking проходят через typed repository/controller и только на presentation boundary адаптируются в legacy map shape | оба active consumer имеют generation guard, immutable points и redacted typed failures; generated direct call из widgets удалён | SECURITY DEFINER без fixed search path; wrapper всегда anonymous; approved/admin, но не active; zoom grid; no hard result limit/order | высокий; characterization: `docs/map_read_contract_characterization.md`, integrations: `docs/home_map_read_integration.md`, `docs/select_parking_map_read_integration.md` |
...
| Google Geocoding; Home/SelectParking | validated `lat`, `lng`; language `en` | typed immutable formatted address; safe empty presentation fallback | provider status/shape validated; HTTP, no-result, invalid-data и unavailable разделены и redacted; credential всё ещё legacy generated transport concern | не Supabase | средний для runtime UX, высокий отдельный config/security debt; `docs/reverse_geocoding_read_integration.md` |

## Загрузка парковок, геопоиск и фильтры

1. `CustomGoogleMap` получает typed `MapMarkerItem`, visible region и zoom от
   Google Maps; Supabase/JSON keys renderer-у неизвестны.
...
| `features/favorites/data/supabase_favorites_repository.dart` через `favourites/favourites/favourites_widget.dart` | `view_user_favorites` | обязательный owner select `user_id = currentUserUid`; пустой user ID не вызывает query | typed `List<FavoriteParkingSummary>` | local controller: loading/empty/failure/retry, stale response guard; cross-user/malformed rows отклоняются | низкий |
| `features/parking_details/data/supabase_parking_details_repository.dart` / initial favorite | `view_full_parking_details` | select by parking `id`; `is_favorited` вычисляется для current user | typed `ParkingDetails` | initial state входит в bounded details read; direct favorite lookup удалён | средний |
| `features/parking_details/data/supabase_parking_favorite_repository.dart` / details toggle | `favorites` | insert `user_id`, `parking_id`; delete by `parking_id` + authenticated `user_id` | `Future<void>` + typed redacted failures | duplicate insert идемпотентен; local controller выполняет single-flight optimistic update и rollback | средний |
