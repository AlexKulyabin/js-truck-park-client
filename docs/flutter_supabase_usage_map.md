...
| `features/favorites/data/favorites_service.dart` / favorites list | `view_user_favorites` | select where `user_id = currentUserUid` | `List<FavoriteParking>` | empty state есть; empty/invalid user id → empty list; error отдельно не показан | средний |
