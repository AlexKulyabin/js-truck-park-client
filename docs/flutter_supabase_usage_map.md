...
| `features/review_submission/data/review_submission_service.dart` / `reviews/review_create/review_create_widget.dart` | `reviews` | no-photo review insert behind guarded `reviewCreate`; photo submissions rejected until compensation flow exists | `PreparedReviewSubmission` / `ReviewSubmissionResult` | test-write capability only; legacy direct review insert removed from button action | высокий |
