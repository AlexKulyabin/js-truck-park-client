...
|---|---|---|---|---|---|
| `features/profile/data/user_profile_service.dart` -> `auth/validate_sms_code/validate_sms_code_widget.dart` / после OTP | `users` | select where `id = currentUserUid` | typed `PublicUserProfile?` / completion bool | empty/invalid user id -> incomplete profile; без catch; выбирает Home или Registration | высокий |
...
| `features/profile/data/user_profile_service.dart` -> `profile/profile/profile_widget.dart` / header | `users` | select where id | `List<PublicUserProfile>` | empty → nullable row/default name; FutureBuilder error не отделён от loading | средне-высокий |
| `features/profile/data/user_profile_service.dart` -> `profile/profile/profile_widget.dart` / referral | `users` | select by id → `referral_code` | `String?` through private profile boundary | existing UI still expects a referral code before creating invite link | высокий |
| `features/profile/data/user_profile_service.dart` -> `profile/edit_profile/edit_profile_widget.dart` / initial form | `users` | select where id, limit 1 | `List<PublicUserProfile>` | FutureBuilder spinner; empty row supported; no explicit DB catch | средне-высокий |
...
| `features/review_submission/data/review_submission_service.dart` (prepared, not wired) | future atomic review + photo boundary | typed command validation only; `reviewCreate` capability always disabled | `PreparedReviewSubmission` / future `ReviewSubmissionResult` | no Supabase implementation; gateway requires atomic execution or full compensation | заблокирован до backend stage |
