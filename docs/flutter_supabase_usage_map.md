...
| `features/review_submission/data/review_submission_service.dart` (not wired) | `reviews` | no-photo review insert behind guarded `reviewCreate`; photo submissions rejected until compensation flow exists | `PreparedReviewSubmission` / `ReviewSubmissionResult` | test-write capability only; no UI wiring yet | высокий |
