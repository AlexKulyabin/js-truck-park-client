...
| `features/profile/data/user_profile_service.dart` -> `profile/profile/profile_widget.dart` / referral | `private_profiles` | select by id → `referral_code` | `String?` through private profile boundary | existing UI still expects a referral code before creating invite link | средний |
| `features/profile/data/user_profile_service.dart` -> `profile/edit_profile/edit_profile_widget.dart` / initial form | `public_profiles` | select where id, limit 1 | `List<PublicUserProfile>` | FutureBuilder spinner; empty row supported; no explicit DB catch | средний |
| `profile/edit_profile/edit_profile_widget.dart` / save | `users` | update `full_name`, optional `avatar_url`, `updated_at` by id | generated update rows | upload and DB update remain separate; no explicit DB catch | высокий |
| `features/profile/data/user_profile_service.dart` (prepared, not wired) | future profile/avatar update boundary | typed command validation only; `profileUpdate` capability disabled | `PreparedUserProfileUpdate` / future `PrivateUserProfile` | no Supabase update gateway; gateway requires atomic execution or full compensation | заблокирован до backend stage |
...
| `features/parking_details/data/parking_details_service.dart` -> parking details and its tabs | `view_full_parking_details` | querySingle where `id = parkingId` | typed `ParkingDetails?` with `ParkingDetailPhoto` payload | empty/invalid id -> null; spinner on load/error; nullable row handling частичное | высокий |
...
| `features/reviews/data/reviews_service.dart` (not wired) | `reviews` | owner-scoped update of `comment` and five rating fields behind guarded `reviewUpdate` | `UpdatedReview` | validates owner id/review id/rating range/content; UI wiring not added yet | высокий |
| `features/reviews/data/reviews_service.dart` (not wired) | `reviews`, `parking_photos`, `parking_content` | owner-scoped review delete behind guarded `reviewDelete`; list review photo URLs, delete review row, then best-effort remove public Storage objects | `DeletedReview` | DB row delete is scoped by `id` + `user_id`; Storage cleanup failures are counted, not retried here | высокий |
...
| `features/review_submission/data/review_submission_service.dart` / `reviews/review_create/review_create_widget.dart` | `reviews`, `parking_photos`, `parking_content` | review insert behind guarded `reviewCreate`; optional staged photo uploads using `parkings/<parkingId>/reviews/<reviewId>/<index>/...` | `PreparedReviewSubmission` / `ReviewSubmissionResult` | validates JPEG/PNG/WebP, <=5 MiB, <=1920px when dimensions are known; compensates created review/object state on failure | высокий |
...
| `features/parking_requests/application/parking_requests_controller.dart` -> `features/parking_requests/data/parking_requests_service.dart` -> requests screen | `parkings` | select by `created_by` + typed pending/approved/rejected status | immutable `ParkingRequestsState` with transitional route-compatible `List<ParkingsRow>` | empty/invalid user id -> empty list; stale tab responses ignored; failure stored while UI preserves spinner behavior; no pagination | средне-высокий |
...
|---|---|---|---|---|
| registration, edit profile | `avatars/users/<uid>/...` | upload, public URL stored in `users.avatar_url` | upload list length проверяется; DB update отдельно; future `profileUpdate` contract remains disabled until atomic/compensated avatar handling exists | bucket public, 5 MiB images; локальная migration ограничивает write/delete owner path, production не изменялась |
