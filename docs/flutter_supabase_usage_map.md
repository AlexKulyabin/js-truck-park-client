...
| `get_filtered_parkings`; `map/home_page/home_page_widget.dart`, `create_parking2/select_parking/select_parking_widget.dart` | POST: ровно 18 params: center/radius, capacity bounds, viewport bounds, six amenity flags, `zoom_level`, lower-case `search_query`, `is_filter_active` | JSON rows: `id/lat/lng/latitude/longitude/count/is_cluster/address/rating`; consumer остаётся `dynamic` | caller местами использует `succeeded ?? true`; malformed item silently skips; camera responses не имеют generation guard | SECURITY DEFINER без fixed search path; wrapper всегда anonymous; approved/admin, но не active; zoom grid; no hard result limit/order | критический; characterization: `docs/map_read_contract_characterization.md` |
