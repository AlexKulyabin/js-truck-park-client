...
| Home map/search | `map/home_page`, `features/map`, `custom_code/widgets/custom_google_map.dart`, legacy `map/map` | local stale-safe controllers + typed repository; filter fields пока в `FFAppState`; legacy marker adapter на UI boundary | REST RPC `get_filtered_parkings`; declared `get_parkings_by_viewport`; public Storage marker URL | Google Maps, Geolocator, Google Geocoding, RevenueCat | generated model/theme/util/custom map сохранены; прямой parking RPC из Home удалён | высокий; Home read-path изолирован, SelectParking и renderer ещё legacy |
